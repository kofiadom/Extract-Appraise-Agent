...
if TYPE_CHECKING:
    from core.byot_schemas import AppraisalTemplate
...
_APPRAISAL_INSTRUCTIONS_TEMPLATE = Template("""You are a specialist research quality appraiser for the Rapid Evidence Synthesis Team (REST).

WORKFLOW — follow these steps exactly for each markdown filename provided:
1. Call read_file(file_name="<filename>.md") to read the full paper content.
2. Evaluate all $criteria_count criteria against the content returned by read_file.

Your task is to evaluate the methodological quality of academic papers using a custom quality assessment framework.

## THE $criteria_count QUALITY CRITERIA
...
- EVERY applicable justification MUST include an inline page/location reference and a verbatim quote from the paper.
- If you cannot find enough information for a criterion, rate it "Partial", state what you could not confirm, and quote the closest available evidence with its page.
- You MUST include all $criteria_count criteria in the output, even those rated "N/A".
- Be balanced — acknowledge both strengths and limitations.
- IMPORTANT: Your final output MUST be a valid JSON object with a single 'appraisals' key containing the list of paper appraisals. DO NOT output prose, markdown, or explanations outside the JSON.
""")


def build_appraisal_instructions(template: AppraisalTemplate) -> str:
    """Build appraisal system instructions from a custom BYOT template."""
    criteria_block = "\n".join(
        f"  {i + 1:2d}. [{', '.join(c.applicability)}] {c.name}: {c.description}"
        for i, c in enumerate(template.criteria)
    )
    return _APPRAISAL_INSTRUCTIONS_TEMPLATE.substitute(
        criteria_count=len(template.criteria),
        criteria_block=criteria_block,
    )


def build_appraisal_prompt(template: AppraisalTemplate) -> str:
    """Build the appraisal runtime prompt from a custom BYOT template."""
    return (
        "For each markdown filename provided: call read_file to read the full paper content, "
        f"then perform systematic quality appraisal of ALL papers using the custom {len(template.criteria)}-criterion framework. "
        "You MUST respond with ONLY a valid JSON object — no prose, no markdown, no explanation.\n"
        '{"appraisals": [{"article_reference": "...", "study_type": "...", '
        '"criteria": [{"criterion_id": 1, "question": "...", "rating": "Yes|Partial|No|N/A", '
        '"justification": "..."}], '
...
def create_filesearch_appraisal_agent(
    ...
    instructions = custom_instructions if custom_instructions is not None else APPRAISAL_INSTRUCTIONS_FS
    return Agent(
