...
export class PipelineController {
  ...
  async run(@Body() body: RunPipelineDto, @CurrentUser() user: AuthUser) {
    const result = await this.pipelineService.runPipeline(user.userId, body.markdownFiles, body.steps, body.fileMapping, body.templateId);
  ...
  async runBatch(@Body() body: RunPipelineDto, @CurrentUser() user: AuthUser) {
    const jobs = await this.pipelineService.runPipelineForFiles(user.userId, body.markdownFiles, body.steps, body.fileMapping, body.templateId);
